...
export interface OverlayLupinApi {
  ...
  /** `gameId`/`variant` are whatever the inviter had selected — both sides start the same game under the same rule. */
  onMatchFound(cb: (peer: PeerInfo, isHost: boolean, gameId: string, variant?: string) => void): void;
